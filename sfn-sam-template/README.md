...
```bash
sfn-sam-template$ sam logs -n StockCheckerFunction --stack-name sfn-sam-template --tail
```
...
```bash
sfn-sam-template$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name sfn-sam-template
```
